...
$student_name = '';
if ($student_id > 0) {
    $table_name = ($student_type === 'jhs') ? 'jhs_students' : 'shs_students';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Debug logging
    error_log("Form submission - Student ID: $student_id, Student Type: $student_type, School Year: $school_year_id");
    error_log("Violations: " . print_r($violations, true));

    if (!empty($violations) && $student_id > 0) {
        ...
        $check_existing_violations_stmt = $conn->prepare("SELECT COUNT(*) as existing_count FROM hs_violations WHERE student_id = ? AND student_type = ? AND school_year_id = ?");
        $check_existing_violations_stmt->bind_param("isi", $student_id, $student_type, $school_year_id);
        ...
        // Get current semester (only for SHS, JHS doesn't use semester)
        $current_semester = getCurrentSemester($conn);

        // Determine which table to insert into based on student type
        if ($student_type === 'shs') {
            // Insert into shs_violations table (SHS uses semester_id)
            $stmt = $conn->prepare("INSERT INTO shs_violations (student_id, student_type, violation_type, description, school_year_id, semester_id, date_recorded) VALUES (?, ?, ?, ?, ?, ?, NOW())");
        } else {
            // Insert into hs_violations table for JHS students (JHS does NOT use semester_id)
            $stmt = $conn->prepare("INSERT INTO hs_violations (student_id, student_type, violation_type, description, school_year_id, date_recorded) VALUES (?, ?, ?, ?, ?, NOW())");
        }
        ...
        if ($stmt) {
            foreach ($violations as $violation) {
                ...
                // Bind parameters differently for SHS (with semester) vs JHS (without semester)
                if ($student_type === 'shs') {
                    $stmt->bind_param("isssii", $student_id, $student_type, $violation, $violation, $school_year_id, $current_semester);
                } else {
                    $stmt->bind_param("isssi", $student_id, $student_type, $violation, $violation, $school_year_id);
                }
                ...
                if (!$stmt->execute()) {
                    ...
                    error_log("Successfully inserted violation: " . $violation . " for student: " . $student_id . " into " . ($student_type === 'shs' ? 'shs_violations' : 'hs_violations') . " table");

                    // Auto-archive this inserted violation and upsert student summary
                    if ($student_type === 'shs') {
                        $archiveSql = "INSERT INTO violation_history_archive
                            ...
                            SELECT s.id,
                                   ...
                                   'shs', s.year_lvl, s.strand, ?, ?, NOW(), ?, ?, ?,
                                   COALESCE(san.sanction_hours, 0)
                            FROM shs_students s
                            LEFT JOIN student_sanctions san ON san.student_id = s.id AND san.student_type = 'shs' AND san.school_year_id = ?
                            WHERE s.id = ?";
                  ...
                  first_violation_date = LEAST(first_violation_date, VALUES(first_violation_date))";
                        ...
                        $archiveSql = "INSERT INTO violation_history_archive
                            (student_id, student_name, student_type, student_level, violation_type,
                             violation_description, date_recorded, school_year_id, school_year, sanction_hours)
                            SELECT s.id,
                                   CONCAT(s.firstName, ' ', COALESCE(s.middleName, ''), ' ', s.lastName),
                                   'jhs', s.year_lvl, ?, ?, NOW(), ?, ?,
                                   COALESCE(san.sanction_hours, 0)
                            FROM jhs_students s
                            LEFT JOIN student_sanctions san ON san.student_id = s.id AND san.student_type = 'jhs' AND san.school_year_id = ?
                            WHERE s.id = ?";
                        if ($arc = $conn->prepare($archiveSql)) {
                            $arc->bind_param('ssisii', $violation, $violation, $school_year_id, $school_year_text, $school_year_id, $student_id);
                            $arc->execute();
                            $arc->close();
                        }
               $sumSql = "INSERT INTO student_summary_archive
                (student_id, student_name, student_type, student_level, school_year_id, school_year,
                 total_violations, total_sanction_hours, first_violation_date)
                SELECT s.id, CONCAT(s.firstName,' ',COALESCE(s.middleName,''),' ',s.lastName),
                    'jhs', s.year_lvl, ?, ?, 1,
                    COALESCE(san.sanction_hours,0), NOW()
                ...
                ON DUPLICATE KEY UPDATE
                  ...
                  first_violation_date = LEAST(first_violation_date, VALUES(first_violation_date))";
                        if ($sum = $conn->prepare($sumSql)) {
                            $sum->bind_param('isii', $school_year_id, $school_year_text, $school_year_id, $student_id);
                            $sum->execute();
                            $sum->close();
                        }
                    }
            ...
            if ($existing_sanction) {
                ...
                // FIX: Removed assigned_by and semester_id (JHS doesn't use semester)
                $update_sanction_stmt = $conn->prepare("UPDATE student_sanctions SET sanction_hours = ?, date_assigned = NOW() WHERE student_id = ? AND student_type = ? AND school_year_id = ? AND status = 'active'");
                $update_sanction_stmt->bind_param("iisi", $new_total_hours, $student_id, $student_type, $school_year_id);
                ...
                // Insert new sanction
                // FIX: Removed assigned_by and semester_id (JHS doesn't use semester)
                $insert_sanction_stmt = $conn->prepare("INSERT INTO student_sanctions (student_id, student_type, sanction_hours, date_assigned, school_year_id, status) VALUES (?, ?, ?, NOW(), ?, 'active')");
                $insert_sanction_stmt->bind_param("isii", $student_id, $student_type, $total_sanction_hours, $school_year_id);
        ...
        header("Location: jhs-list.php?success=1");
        exit();
    } else {
        // Log error for debugging
        if (empty($violations)) {
            error_log("No violations selected");
        }
        if ($student_id <= 0) {
        ...
        header("Location: jhs-list.php?error=1");
        exit();
    }
}
?>
...
<head>
    ...
    <title>HS Violation Selection</title>
    <link rel="stylesheet" href="../styles/style.css">
    <script src="../scripts/script.js"></script>
</head>
<body>
    <div class="icon-back-wrapper">
        <a href="jhs-list.php" class="icon-back-btn" aria-label="Back to Records">
    ...
    <h3>Please Select Student Violation/s</h3>
    <form method="POST" id="violationForm">
        ...
        <div class="violation-options">
            <div class="violation-card">
                <h4>Improper School Uniform</h4>
                <ul>
                    <li>No Pin</li>
                    <li>No necktie/ribbon</li>
                    <li>Wearing of white socks</li>
                    <li>Not wearing appropriate shoes</li>
                </ul>
                <input type="checkbox" name="violations[]" value="Improper School Uniform" id="improper_school_uniform" class="violation-checkbox">
                <label for="improper_school_uniform" class="violation-label">Select Violation</label>
            </div>

            <div class="violation-card">
                <h4>Improper PE Uniform</h4>
                <ul>
                    <li>Not wearing complete PE Uniform</li>
                    <li>Not wearing NST shirt/Black Pants</li>
                </ul>
                <input type="checkbox" name="violations[]" value="Improper PE Uniform" id="improper_pe_uniform" class="violation-checkbox">
                <label for="improper_pe_uniform" class="violation-label">Select Violation</label>
                </div>
            ...
            <div class="violation-card">
                <h4>No ID</h4>
                <p>Not wearing school ID</p>
                <input type="checkbox" name="violations[]" value="No ID" id="no_id" class="violation-checkbox">
                <label for="no_id" class="violation-label">Select Violation</label>
            </div>
