...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($violations) && $student_id > 0) {
        ...
        $check_existing_violations_stmt = $conn->prepare("SELECT COUNT(*) as existing_count FROM shs_violations WHERE student_id = ? AND school_year_id = ?");
        ...
        if ($student_type === 'shs') {
            // Insert into shs_violations table
            ...
            // Insert into hs_violations table for JHS students
            $stmt = $conn->prepare("INSERT INTO hs_violations (student_id, student_type, violation_type, description, school_year_id, semester_id, date_recorded) VALUES (?, ?, ?, ?, ?, ?, NOW())");
        }
        ...
        if ($stmt) {
                ...
                if (!$stmt->execute()) {
                    ...
                    error_log("Successfully inserted violation: " . $violation . " for student: " . $student_id . " into " . ($student_type === 'shs' ? 'shs_violations' : 'hs_violations') . " table");
	                    ...
	                    if ($student_type === 'shs') {
	                        ...
	                        $sumSql = "INSERT INTO student_summary_archive
	                            (student_id, student_name, student_type, student_level, school_year_id, semester_id, school_year,
	                             total_violations, total_sanction_hours, first_violation_date, last_violation_date)
	                            SELECT s.id, CONCAT(s.firstName,' ',COALESCE(s.middleName,''),' ',s.lastName),
	                                   'jhs', s.year_lvl, ?, ?, ?, 1,
	                                   COALESCE(san.sanction_hours,0), NOW(), NOW()
            ...
            if ($existing_sanction) {
                ...
                $update_sanction_stmt->bind_param("iiisi", $new_total_hours, $current_semester, $student_id, $student_type, $school_year_id);
        ...
        header("Location: shs-list.php?success=1");
        exit();
    } else {
        ...
        header("Location: shs-list.php.php?error=1");
        exit();
    }
}
?>
...
<body>
    <div class="icon-back-wrapper">
        <a href="shs-list.php" class="icon-back-btn" aria-label="Back to Records">
